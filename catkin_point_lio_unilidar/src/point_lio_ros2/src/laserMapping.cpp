...
// Fixed camera_init frame: capture FULL initial rotation of the lidar
SO3 rot_init_first;  // camera_init has the FIRST orientation of the lidar (fixed)
bool gravity_align_first_time = true;
...
void publish_init_kdtree(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes) {
    ...
    // Apply 180° rotation around X axis: x'=x, y'=-y, z'=-z
    laserCloudInit->points.resize(ikdtree.PCL_Storage.size());
    for (size_t i = 0; i < ikdtree.PCL_Storage.size(); i++) {
        laserCloudInit->points[i].x = ikdtree.PCL_Storage[i].x;
        laserCloudInit->points[i].y = -ikdtree.PCL_Storage[i].y;
        laserCloudInit->points[i].z = -ikdtree.PCL_Storage[i].z;
        laserCloudInit->points[i].intensity = ikdtree.PCL_Storage[i].intensity;
    }
...
void transform_to_baselink(const geometry_msgs::msg::Pose& body_pose,
                           geometry_msgs::msg::Pose& baselink_pose) {
    // Extract body pose
    Eigen::Vector3d pos_body(body_pose.position.x,
                             body_pose.position.y,
                             body_pose.position.z);
    Eigen::Quaterniond quat_body(body_pose.orientation.w,
                                 body_pose.orientation.x,
                                 body_pose.orientation.y,
                                 body_pose.orientation.z);
    Eigen::Matrix3d R_body = quat_body.toRotationMatrix();

    // Transform: T_world_baselink = T_world_body * T_body_baselink
    Eigen::Vector3d pos_baselink = pos_body + R_body * Baselink_T_wrt_Body;
    Eigen::Matrix3d R_baselink = R_body * Baselink_R_wrt_Body;
    Eigen::Quaterniond quat_baselink(R_baselink);
    quat_baselink.normalize();

    // Set output
    baselink_pose.position.x = pos_baselink.x();
    baselink_pose.position.y = pos_baselink.y();
    baselink_pose.position.z = pos_baselink.z();
    baselink_pose.orientation.w = quat_baselink.w();
    baselink_pose.orientation.x = quat_baselink.x();
    baselink_pose.orientation.y = quat_baselink.y();
    baselink_pose.orientation.z = quat_baselink.z();
}

void transform_twist_to_baselink(const geometry_msgs::msg::Twist& body_twist,
                                 geometry_msgs::msg::Twist& baselink_twist,
                                 const Eigen::Quaterniond& body_orientation) {
    // Extract velocities
    Eigen::Vector3d lin_vel_body(body_twist.linear.x,
                                 body_twist.linear.y,
                                 body_twist.linear.z);
    Eigen::Vector3d ang_vel_body(body_twist.angular.x,
                                 body_twist.angular.y,
                                 body_twist.angular.z);

    // Transform considering rotation and velocity offset
    Eigen::Vector3d lin_vel_baselink = Baselink_R_wrt_Body * lin_vel_body +
                                       ang_vel_body.cross(Baselink_T_wrt_Body);
    Eigen::Vector3d ang_vel_baselink = Baselink_R_wrt_Body * ang_vel_body;

    // Set output
    baselink_twist.linear.x = lin_vel_baselink.x();
    baselink_twist.linear.y = lin_vel_baselink.y();
    baselink_twist.linear.z = lin_vel_baselink.z();
    baselink_twist.angular.x = ang_vel_baselink.x();
    baselink_twist.angular.y = ang_vel_baselink.y();
    baselink_twist.angular.z = ang_vel_baselink.z();
}
...
void publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped,
    ...
    // Transform to base_link if enabled
    if (publish_in_baselink_frame) {
        geometry_msgs::msg::Pose body_pose = odomAftMapped.pose.pose;
        geometry_msgs::msg::Twist body_twist = odomAftMapped.twist.twist;

        Eigen::Quaterniond body_quat(body_pose.orientation.w,
                                     body_pose.orientation.x,
                                     body_pose.orientation.y,
                                     body_pose.orientation.z);

        transform_to_baselink(body_pose, odomAftMapped.pose.pose);
        transform_twist_to_baselink(body_twist, odomAftMapped.twist.twist, body_quat);
    ...
    // Apply 180° rotation around X axis to transform from lidar frame (Z down) to world frame (Z up)
    // Rx(180°) = [1, 0, 0; 0, -1, 0; 0, 0, -1]
    // For position: x' = x, y' = -y, z' = -z
    // For rotation: R' = Rx(180°) * R
    Eigen::Matrix3d rot_180x;
    rot_180x << 1,  0,  0,
                0, -1,  0,
                0,  0, -1;

    // Transform position
    double pos_x = odomAftMapped.pose.pose.position.x;
    double pos_y = -odomAftMapped.pose.pose.position.y;  // Invert Y
    double pos_z = -odomAftMapped.pose.pose.position.z;  // Invert Z

    // Transform rotation
    SO3 rot_kf = use_imu_as_input ? kf_input.x_.rot : kf_output.x_.rot;
    SO3 rot_flipped = SO3(rot_180x) * rot_kf;
    Eigen::Quaterniond quat_flipped(rot_flipped.matrix());

    // Update odometry message with transformed pose
    odomAftMapped.pose.pose.position.x = pos_x;
    odomAftMapped.pose.pose.position.y = pos_y;
    odomAftMapped.pose.pose.position.z = pos_z;
    odomAftMapped.pose.pose.orientation.w = quat_flipped.w();
    odomAftMapped.pose.pose.orientation.x = quat_flipped.x();
    odomAftMapped.pose.pose.orientation.y = quat_flipped.y();
    odomAftMapped.pose.pose.orientation.z = quat_flipped.z();
    ...
    transform.transform.rotation.w = quat_flipped.w();
    transform.transform.rotation.x = quat_flipped.x();
    transform.transform.rotation.y = quat_flipped.y();
    transform.transform.rotation.z = quat_flipped.z();
...
void publish_path(const rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr &pubPath) {
    ...
    // Apply 180° rotation around X axis (same as odometry)
    ...
    SO3 rot_flipped = SO3(rot_180x) * rot_kf;
    Eigen::Quaterniond quat_flipped(rot_flipped.matrix());
    msg_body_pose.pose.orientation.w = quat_flipped.w();
    msg_body_pose.pose.orientation.x = quat_flipped.x();
    msg_body_pose.pose.orientation.y = quat_flipped.y();
    msg_body_pose.pose.orientation.z = quat_flipped.z();
...
int main(int argc, char **argv) {
    ...
    Baselink_T_wrt_Body << VEC_FROM_ARRAY(body_to_baselink_T);
    Baselink_R_wrt_Body << MAT_FROM_ARRAY(body_to_baselink_R);
    if (extrinsic_est_en) {
        if (!use_imu_as_input) {
            ...
            if (imu_en) {
                if (!p_imu->gravity_align_) {
                    ...
                    if (gravity_align) {
                        ...
                        // Capture FULL initial rotation for camera_init on first initialization
                        if (gravity_align_first_time) {
                            rot_init_first = state_out.rot;  // Store COMPLETE rotation
                            gravity_align_first_time = false;
                        }
                    }
